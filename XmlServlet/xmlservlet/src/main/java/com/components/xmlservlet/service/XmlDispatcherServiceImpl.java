...
public class XmlDispatcherServiceImpl implements XmlDispatcherService {
	...
	@Autowired
	public XmlDispatcherServiceImpl(final CustomerRepository repo, XmlConverter converter) {
		this.repo = repo;
	...
	public String doService(String xmlRequest) {
		...
		try {
			...
			if (method.equals(CREATE)) {
				...
				repo.saveAndFlush(new Customer(xmlMap.get(EMAILADDRESS), xmlMap.get(PASSWORD)));
				serviceResponse.setRequestId(xmlMap.get(REQUESTID));
			...
			if (method.equals(DELETE)) {
				...
				Optional<Customer> dbService = repo.findByEmailAddress(xmlMap.get(EMAILADDRESS));
